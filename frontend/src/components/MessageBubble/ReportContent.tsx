...
import {
  SKIP_FIELDS, SCORE_OBJECT_FIELDS, SCORE_FIELDS, SCORECARD_KEY,
  NARRATIVE_FIELDS, HEADLESS_NARRATIVE_FIELDS,
  isEmptyValue, isFlatObject, isSimpleList, getSimpleListLabel,
  sortAndFilterEntries, labelFromKey,
} from './reportUtils';
...
function formatCellValue(value: unknown): React.ReactNode {
  ...
  if (typeof value === 'string') {
    ...
    if (severityClass) return <span className={`${styles.arrayCardBadge} ${severityClass}`}>{value}</span>;
    return value;
  }
...
function formatArrayValue(key: string, value: unknown[]): React.ReactNode {
  ...
  return (
    <ul className={styles.primitiveList}>
      {value.map((item, i) => {
        const text = typeof item === 'string' || typeof item === 'number' ? String(item) : JSON.stringify(item);
...
export function formatValue(key: string, value: unknown): React.ReactNode {
  ...
  if (typeof value === 'string') {
    if (NARRATIVE_FIELDS.has(key) && value.trim()) {
      return <div className={styles.markdownContent}><ReactMarkdown>{value}</ReactMarkdown></div>;
    }
    return value;
  }
...
const EXCLUDED_TABLE_KEYS = new Set([
  'observation', 'impact', 'remediation', 'threat_description',
  'attack_vector', 'mitigation_strategy', 'why_it_blocks_scaling', 'fix',
]);

interface ObjectTableProps { readonly items: Record<string, unknown>[]; }

function ObjectTable({ items }: ObjectTableProps) {
  const allKeys = Array.from(
    new Set(items.flatMap(item => Object.keys(item).filter(k => !EXCLUDED_TABLE_KEYS.has(k))))
  );
  return (
    <div className={styles.tableWrapper}>
      <table className={styles.dataTable}>
        <thead>
          <tr>{allKeys.map(k => <th key={k}>{labelFromKey(k).toUpperCase()}</th>)}</tr>
        </thead>
        <tbody>
          {items.map((item, i) => {
            const rowKey = typeof item.id === 'string' ? item.id : `row-${i}`;
            return (
              <tr key={rowKey}>
                {allKeys.map(k => <td key={k}>{formatCellValue(item[k])}</td>)}
              </tr>
            );
          })}
        </tbody>
...
// --- FlatPropertyGrid ---
interface FlatPropertyGridProps { readonly obj: Record<string, unknown>; }
...
interface ScoreCardsProps { readonly report: Record<string, unknown>; }
...
// --- ScorecardHero ---
interface ScorecardHeroProps { readonly scorecard: Record<string, unknown>; }

function ScorecardHero({ scorecard }: ScorecardHeroProps) {
  const health = typeof scorecard.architecture_health === 'string' ? scorecard.architecture_health : '';
  const risk = typeof scorecard.primary_risks === 'string' ? scorecard.primary_risks : '';
  const bottleneck = typeof scorecard.primary_bottleneck === 'string' ? scorecard.primary_bottleneck : '';
  return (
...
// --- ReportContent ---
export interface ReportContentProps { readonly report: Record<string, unknown>; }
  ...
  return (
    <div className={styles.reportContent}>
      ...
      {entries.map(([key, value]) => {
        if (SKIP_FIELDS.has(key) || isEmptyValue(value) || SCORE_OBJECT_FIELDS.has(key)) return null;
        ...
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
          ...
          if (key === SCORECARD_KEY) {
            return <div key={key} className={styles.reportSection}><ScorecardHero scorecard={obj} /></div>;
          }
          return (
            <div key={key} className={styles.reportSection}>
